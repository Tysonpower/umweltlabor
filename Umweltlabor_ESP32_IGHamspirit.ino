...
#define WIFI_PASSWORD "password"                        // AP Password
#define MQTT_SERVER "fra4.tynet.eu"                     // MQTT Server
#define MQTT_USER "myUser"                              // MQTT User     (if any)
#define MQTT_PASS "myUserPassword"                      // MQTT Password (if any)
...
#define TEMT_PIN 15         // TEMT6000 PIN for Analog Read
...
void setup() {
  Serial.begin(115200);
  Serial.println("Umweltlabor Dreetzsee Projekt - IG Hamspirit");
...
void loop() {
    ...
    if (currentMillis - previousMillis > MEASURE_DELAY*1000) {
      ...
      Serial.print(map(temtValue, 0, 4096, 0, 1000));
      Serial.println(" lux");

      int sensorValue = analogRead(15);
      // print out the value you read:
      Serial.println(map(sensorValue, 0, 4096, 0, 1000));
